...
// Whether auth is usable. Only SESSION_SECRET is strictly required — the
// password chain (db → env → built-in first-run default) lives in authstore.
function configured() {
  return !!secret();
}
...
// `extra` merges additional claims into the JWT (e.g. mustChange).
function issueSession(res, extra) {
  const csrf = b64url(crypto.randomBytes(24));
  const now = Math.floor(Date.now() / 1000);
  const token = sign(Object.assign({ sub: 'admin', role: 'admin', csrf, iat: now, exp: now + SESSION_TTL }, extra || {}));
