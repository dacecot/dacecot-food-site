...
async function sendRescheduled(sub, oldDate) {
  const newDate = (sub.details && (sub.details.class_date || sub.details.pickup_day)) || 'a new date';
  const intro =
    '<p style="font-size:15px;color:#2b2b2b;margin:0">' + greeting(sub) + '</p>' +
...
module.exports = { sendPaymentLink, sendReminder, sendCancelled, sendRescheduled, BRAND };
